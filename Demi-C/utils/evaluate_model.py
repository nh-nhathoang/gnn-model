...
def visualize_performance(y_K, pred_K, R2_K, acc_K, cover_interval, overlap, save_dir):
    ...
    ax.set_ylim([min(pred_K), max(pred_K)])
    ax.legend()

    plt.tight_layout()
    
    print('Accuracy for E', np.mean(acc_K))
...
def evaluate_model(model, loader, device, cover_interval, overlap, save_dir):
    ...
    visualize_performance(y_whole_K, K_pred_whole, R2_K, acc_K_total, cover_interval, overlap, save_dir)
    return R2_K
